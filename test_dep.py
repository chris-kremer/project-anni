    ...
    if os.path.exists(data_file_path):
        try:
            with open(data_file_path, "r") as file:
                data = json.load(file)
                return data.get("ownership", {"Percentage": 0.3225979})
...
def fetch_historical_prices(tickers):
    tickers.append("SPY")  # Ensure SPY is always included
    historical_prices = {}
...
def calculate_weekly_share_value(portfolio, historical_prices, ownership, initial_cash):
    ...
    for date in all_dates:
        ...
        if "SPY" in historical_prices and date in historical_prices["SPY"]:
            spy_value = historical_prices["SPY"].loc[date]

        share_value = total_value * (ownership["Percentage"] / 100)
        if share_value >= 500 and spy_value is not None:
            weekly_values.append({"Date": date, "Portfolio": share_value, "SPY": spy_value})

    df = pd.DataFrame(weekly_values)
    df["Date"] = pd.to_datetime(df["Date"])
    df.set_index("Date", inplace=True)

    # Normalize both Portfolio and SPY values to start at 100
    df["Portfolio"] = (df["Portfolio"] / df["Portfolio"].iloc[0]) * 100
    df["SPY"] = (df["SPY"] / df["SPY"].iloc[0]) * 100
...
def main():
    ...
    # Chart: Portfolio vs. SP500 Performance (Base=100)
    st.subheader("Portfolio vs. SP500 Performance (Base=100)")
    ...
    if not weekly_share_value.empty:
        st.line_chart(weekly_share_value, use_container_width=True)
    else:
